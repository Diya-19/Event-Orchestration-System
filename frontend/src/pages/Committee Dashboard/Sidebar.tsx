...
const NAV = [
  ...
  { to: "/dashboard/submission", label: "Submission", icon: <FolderKanban size={18} /> },
...
export default function DashboardLayout() {
  ...
  // Fetch the event whenever the eventId in the URL changes
  useEffect(() => {
    if (!eventId) { 
  ...
  // Persist the event_id parameter across all navigation links
      ...
      <aside className="w-[210px] bg-white border-r border-[#ece8f3] flex flex-col justify-between shrink-0">
        ...
        <div>
          ...
          <div className="px-5 py-3 border-b border-[#f2edf8]">
            {selectedEvent ? (
              <div className="p-3 bg-[#f3e8ff] rounded-xl border border-[#e9d5ff]">
                ...
                <p className="text-[13px] font-bold text-[#4c1d95] truncate mt-0.5">{selectedEvent.name}</p>
                <button
          ...
          <nav className="px-3 py-4 space-y-2">
            {NAV.map((item) => (
              <NavLink
                key={item.to}
                to={navTo(item.to)} // INJECTED dynamic routing here
                className={({ isActive }) => `
        ...
        <div className="p-4 border-t border-[#f2edf8]">
          <div className="flex items-center justify-between">
            ...
            <button onClick={logout} className="text-[#667085] hover:text-red-500 transition">
              <LogOut size={18} />
            </button>
          </div>
        </div>
